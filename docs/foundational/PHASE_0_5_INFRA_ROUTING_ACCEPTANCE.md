...
Prereqs: filesystem writable; routing registry endpoints available; surface normalization helper deployed (aliases: squared/squared2/SQUARED2/SQUARED² → canonical).
...
```
Expect route persisted; restart server and GET returns same.
...
## Env removal check
Unset related env vars (e.g., CHAT_BUS_BACKEND, STREAM_TIMELINE_BACKEND, STORAGE_BUCKET); routes still function via registry-configured filesystem backends.
