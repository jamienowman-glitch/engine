...
### audio_render
- Path: `engines/audio_render`
- Interface: library service; render controllers still call it directly (no public HTTP route yet).
- Description: Builds deterministic ffmpeg mix plans from timeline sequences, applies mix bus presets/automation, and emits master + stem outputs.
- Artifacts & data: Writes `audio_render` (master mix) and `audio_bus_stem` artifacts; metadata captures `bus_id`, `roles`, `gain_db`, `export_preset`, `limiter_thresh`, `headroom_db`, `loudnorm_target`, and `dithered`.
- Multi-tenant / identity: tenant_id/env carried through the sequence/request models.
- Status: prod-lite (deterministic ffmpeg plan, guardrails, tests for automation/fade/presets).
- Dependencies: ffmpeg, media_v2, audio_mix_buses.
- Tests: `engines/audio_render/tests` (render plans, mix presets, stem uploads).
- Gaps / TODOs: HTTP/orchestration surface, ffmpeg error handling, upstream job/retry layer.
- Ready for UI?: yes (backend-ready, waiting for HTTP endpoint).
- Notes: `RenderRequest.export_preset` must be one of `default/podcast/music/voiceover`; unknown presets now raise `ValueError` before touching ffmpeg so callers can fail fast.

### audio_timeline
- Path: `engines/audio_timeline`
- Interface: library service; automation/fades used by render/arrangement layers via direct imports.
- Description: Declares `AudioClip`, `AudioTrack`, and `AudioSequence` with fades, crossfades, automation maps (gain/pan/filter) so renders can sample deterministic keyframes; services clamp fade totals to clip duration and reject automation points outside the clip window or duplicated times.
- Artifacts & data: Stores timeline projects/tracks in-memory (or Firestore) and carries automation/fade metadata in `AudioClip`/`AudioTrack`; sequences are referenced by render plans and arrangement/structure engines.
- Multi-tenant / identity: tenant/env stored on sequences; arrangement/structure requests propagate context.
- Status: production (tests cover automation, fades, arrangement integration).
- Dependencies: none beyond the standard timeline stack.
- Tests: `engines/audio_timeline/tests`.
...
### audio_fx_chain
- Path: `engines/audio_fx_chain`
- Interface: service (no HTTP surface). Applies curated FX preset graphs to art./assets.
- Description: Clamps knobs/dry-wet, exposes saturation/delay/reverb/widener presets, logs sanitized overrides.
- Artifacts & data: Emits `audio_sample_fx` artifacts with `params_applied`, `preset_metadata`, and `knob_overrides` metadata for downstream lineage.
- Tests: `engines/audio_fx_chain/tests`.

### audio_macro_engine
- Path: `engines/audio_macro_engine`
- Interface: service; compiles macro graphs and runs ffmpeg filter_complex.
- Description: Provides macro presets (reverse, distortion, reverb) with knob overrides validated/clamped before execution.
- Artifacts & data: Registers `audio_macro` artifacts containing the sanitized `knob_overrides` map and proc meta.
- Tests: `engines/audio_macro_engine/tests`.

### audio_resample
- Path: `engines/audio_resample`
- Interface: service (no HTTP); call when bpm/pitch shifts are needed.
- Description: Resamples with rubberband/soxr including draft/quality presets plus optional formant preservation and clamp guards.
- Artifacts & data: Writes `audio_resampled` artifacts annotated with `resample_params`, `quality_preset`, and `preserve_formants` for tracing; when no tempo/pitch changes were requested the artifact meta also surfaces `returned_original=true` and `reason`.
- Tests: `engines/audio_resample/tests`.
...
- Ready for UI?: yes (dev-only).
- Notes: descriptors now expose `quality_score`, `role`, `brightness`, and `key_root`; query filters honor quality/brightness/key/role (plus BPM/bars) before pagination so downstream flows see deterministic samples.
...
- Ready for UI?: no (needs endpoint).
 - Notes: summary_meta now contains deterministic cache keys plus per-sample `score`/`offset_ms` so downstream pattern engines can rely on scored hit/loop/phrase lists.
...
### image_core
- Path: `engines/image_core`
- Interface: library service (HTTP hook pending if a UI render endpoint surfaces).
- Description: Composes layered canvases (assets/text/vector shapes or solid fills) with deterministic transforms, masks, blend modes, opacity, transform, exposure/brightness/contrast/saturation/sharpness/gamma adjustments, and blur/sharpen filters before exporting PNGs.
- Artifacts & data: Writes `image_render` artifacts via media_v2; metadata now tracks `width`, `height`, `layers_count`, `blend_modes`, `background_color`, a `pipeline_hash` to deduplicate renders, and export preset metadata (`preset_id`, `format`). Uses `vector_scene` and `text_render` kinds for vector/text artifacts where applicable.
  - Built-in export presets (50+ preset IDs):
    - **Web/General:** `web_small` (640w WEBP q75), `web_medium` (1280w WEBP q85), `web_large` (2048w WEBP q90), `original` (no resize PNG)
    - **Social Feed:** `instagram_1080` (1080×1080 JPEG q90), `social_1080p` (1920×1080 PNG), `twitter_card_1200x675` (JPEG q85), `linkedin_banner_1584x396` (JPEG q85), `facebook_cover_820x312` (JPEG q85), `pinterest_pin_1000x1500` (JPEG q85)
    - **Stories/Short-Form:** `instagram_story_1080x1920` (WEBP q85), `tiktok_video_1080x1920` (WEBP q80), `snapchat_story_1080x1920` (WEBP q75), `youtube_thumbnail_1280x720` (JPEG q90)
    - **Email/Newsletters:** `email_header_600x200` (JPEG q85), `email_body_600x400` (JPEG q80), `newsletter_banner_640` (WEBP q85), `web_og_image_1200x630` (JPEG q90)
    - **E-Commerce:** `ecommerce_product_400x400` (PNG), `ecommerce_product_800x800` (PNG), `ecommerce_gallery_1200x1200` (JPEG q95)
    - **Icons & Avatars:** `avatar_64x64` (PNG), `avatar_128x128` (PNG), `icon_256x256` (PNG), `icon_512x512` (PNG), `thumbnail_200` (PNG)
    - **Displays:** `desktop_1920x1080` (WEBP q90), `desktop_2560x1440` (WEBP q92), `ultrawide_3440x1440` (WEBP q85), `retina_display_2x` (WEBP q95), `hero_1920x1080` (PNG), `mobile_small_640x360` (WEBP q75)
    - **Ads & Banners:** `google_ads_300x250` (JPEG q85), `google_ads_728x90` (JPEG q85), `facebook_ads_1200x628` (JPEG q85), `banner_970x250` (JPEG q85)
    - **Print (300dpi):** `print_a4_300dpi` (JPEG 2480×3508 q95), `print_a3_300dpi` (JPEG 3508×4961 q95), `print_postcard_4x6_300dpi` (JPEG 1200×1800 q95), `print_business_card_3_5x2_300dpi` (JPEG 1050×700 q95), `print_poster_18x24_300dpi` (JPEG 5400×7200 q95), `print_300dpi` (JPEG 3000×3000 q95)
    - **Draft/Preview:** `draft_preview_480` (WEBP q60), `draft_preview_720` (WEBP q70)
    - **Archive:** `tiff_high_quality` (TIFF no resize)
- Multi-tenant / identity: tenant/env on composition models; service validates dimensions/opacity prior to rendering.
- Status: prod-lite (deterministic backend, mask artifact support, caching keyed by pipeline hash, tests).
- Dependencies: Pillow, typography_core, vector_core, media_v2.
- Tests: `engines/image_core/tests/test_i01_composite.py`, `engines/image_core/tests/test_i02_masks.py`, `engines/image_core/tests/test_presets.py`.
- Gaps / TODOs: HTTP render endpoint, cache persistence across nodes, richer preset metadata.
- Ready for UI?: yes (artifact metadata exposes blend_modes/pipeline_hash for downstream UX).
...
- Multi-tenant / identity: none.
- Status: prototype -> progress on AV04 (motion library + FK/IK mixing implemented; chain IK solver added and hardened with deterministic convergence tests).
- Dependencies: math utils.
- Tests: `engines/animation_kernel/tests` (motion library playback/blend, IK convergence, FK/IK masks all passing locally).
- Gaps / TODOs: Further IK hardening for edge cases, persistence, tenant/auth, HTTP.
- Ready for UI?: no.
