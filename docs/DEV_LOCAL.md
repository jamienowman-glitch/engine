...
These scripts start the Firestore emulator, seed routing/identity data, and run the FastAPI app on `:8000`.
...
- Runs `uvicorn engines.chat.service.server:app --host 0.0.0.0 --port 8000`

### Verify
In another shell:
```bash
curl -s -o /dev/null -w "%{http_code}\n" http://localhost:8000/ops/status
curl -s -o /dev/null -w "%{http_code}\n" -X POST http://localhost:8000/api/auth/ticket \
