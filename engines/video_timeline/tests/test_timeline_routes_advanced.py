...
def test_trim_route(client):
    ...
    # Preset
    p = svc.create_project(VideoProject(tenant_id="t1", env="dev", title="P"))
    s = svc.create_sequence(Sequence(tenant_id="t1", env="dev", project_id=p.id, name="S"))
    t = svc.create_track(Track(tenant_id="t1", env="dev", sequence_id=s.id, kind="video"))
    c = svc.create_clip(Clip(tenant_id="t1", env="dev", track_id=t.id, asset_id="a1", in_ms=0, out_ms=1000, start_ms_on_timeline=0))
