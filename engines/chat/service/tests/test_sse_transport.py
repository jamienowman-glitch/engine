...
from engines.chat.service.sse_transport import _sse_context, event_stream, router as sse_router
...
async def _collect_event_from_stream(
    thread_id: str, request_context: RequestContext, last_event_id: str | None
):
...
def test_sse_event_stream_resume():
    ...
    chunk = asyncio.run(_collect_event_from_stream(thread_id, ctx, last_event_id=first.id))
