"""Run Memory service with routing-based backend resolution (Agent A - A-4).

Routes run_memory resource_kind through routing registry.
...
class RunMemoryService:
    """Resolves and uses run_memory backend via routing registry.
    ...
    def _resolve_adapter(self):
        """Resolve run_memory backend via routing registry."""
        try:
            ...
            if not route:
                # Fallback to blackboard_store if run_memory not found (backward compatibility)
                route = registry.get_route(
            ...
            if not route:
                raise MissingRoutingConfig(
                    f"No route configured for run_memory in {self._context.tenant_id}/{self._context.env}. "
            ...
            if backend_type == "firestore":
                ...
                return FirestoreRunMemory(project=project)
            elif backend_type == "dynamodb":
                # Cloud: DynamoDB
                table_name = config.get("table_name", "run_memory")
                region = config.get("region", "us-west-2")
                return DynamoDBRunMemory(table_name=table_name, region=region)
            elif backend_type == "cosmos":
                # Cloud: Cosmos
                endpoint = config.get("endpoint")
                key = config.get("key")
                database = config.get("database", "run_memory")
                return CosmosRunMemory(endpoint=endpoint, key=key, database=database)
            else:
                raise RuntimeError(
                    f"Unsupported run_memory backend_type='{backend_type}'. "
    ...
    def write(
        ...
        """Write a value to shared run memory with optimistic concurrency.
        ...
        try:
            ...
            logger.error(f"Run Memory write failed for key '{key}': {exc}")
            raise
    ...
    def read(
        ...
        """Read a value from shared run memory (latest or specific version).
        ...
        try:
            ...
            logger.error(f"Run Memory read failed for key '{key}': {exc}")
            return None
    ...
    def list_keys(self, run_id: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Run Memory list_keys failed for run {run_id}: {exc}")
            return []
